...
Get started quickly with our default token authentication to log in to {variables.prodname} Manager UI and Kibana.

## Concepts

### Authentication defaults

Token authentication is the default authentication option for {variables.prodname} Manager. When a service account is created, an
...
- **{variables.prodname} Manager:** `https://<host>:<port>/login/token`.
- **Kibana:** `https://<host>:<port>/tigera-kibana/login`.

## Before you begin

Make sure you have installed {variables.prodname} using one of the [installation guides](../../getting-started/index.mdx) and have set up [access to the Manager UI](access-the-manager.mdx).
...
Give the service account permissions to access the {variables.prodname} Manager UI, and a {variables.prodname} cluster role:
...
- **role_name** is one of the default cluster roles (or a custom cluster role) specifying {variables.prodname} UI permissions.
...
Now that you have the token, log in to the {variables.prodname} UI and submit the token.

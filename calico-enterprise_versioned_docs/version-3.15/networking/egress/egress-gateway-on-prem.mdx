...
Identify the pull secret that is needed for pulling {variables.prodname} images, and copy this into the
  ...
  template:
    ...
    spec:
      ...
      - name: egress-gateway-init
        ...
        image: {variables.registry}%% component_image("egress-gateway") %%
        env:
...
kubectl apply -f {variables.filesUrl}/manifests/rancher/custom-psp-rke2-cis-hardened.yaml
</CodeBlock>

Set the `serviceAccount` field in the `egress-gateway` `Deployment` to use the created `ServiceAccount`: `tigera-egress-gateway` associated with the `PodSecurityPolicy`,
...
- Before upgrading egress gateways to a particular version, upgrade the other {variables.prodname} components
  first. The egress gateway image should never be newer than the other {variables.prodname} components. We recommend
...
- Ensure that the policy sync API is enabled; this is required by egress gateway images starting with v3.11.0. To
  ...
  allows the egress gateway daemon inside the egress gateway pod to query {variables.noderunning} for the set of active

...
metadata:
  ...
  namespace: <egress gateway namespace>
spec:
  logSeverity: "Info"
  replicas: 2
  ipPools:
...
As of v3.16.3, the operator automatically configures egress gateways to use the same iptables backend as calico-node.
This is controlled by the `iptablesBackend` field in the FelixConfiguration.  If not specified or set to `auto`,
the egress gateway will attempt to autodetect the correct value preferring nft if available.

Prior to v3.16.3, it was not possible to set the iptables backend; the effective value was fixed at `legacy`.
...
```bash
kubectl annotate ns <namespace> egress.projectcalico.org/namespaceSelector="projectcalico.org/name == '<egress gateway namespace>'"
```
...
metadata:
  annotations:
    ...
    egress.projectcalico.org/namespaceSelector: projectcalico.org/name == '<egress gateway namespace>'
  name: my-client,
  namespace: <namespace>,
spec:
  ...

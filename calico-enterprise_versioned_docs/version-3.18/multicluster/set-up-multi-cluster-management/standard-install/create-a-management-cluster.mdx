...
Create a {variables.prodname} management cluster to manage multiple clusters from a single management plane.

## Value

Managing standalone clusters and multiple instances of Elasticsearch is not onerous when you first install {variables.prodname}. But as you move to production with 300+ clusters, it is not scalable; you need centralized cluster management and log storage. With {variables.prodname} multi-cluster management, you can securely connect multiple clusters from different cloud providers in a single management plane, and control user access using RBAC. This architecture also supports federation of network policy resources across clusters, and lays the foundation for a “single pane of glass.”
...
- {variables.prodname} Manager user interface
...
- To change an existing {variables.prodname} standalone cluster to a management or managed cluster, see [Change cluster types](../../change-cluster-type.mdx)

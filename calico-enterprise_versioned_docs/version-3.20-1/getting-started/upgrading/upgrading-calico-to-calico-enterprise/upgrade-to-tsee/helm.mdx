...
Upgrading to {variables.prodname} v3.13 and above, from Calico 3.22 and below is currently unsupported.
...
## Upgrade from Calico to {variables.prodname}
...
1. Install the {variables.prodname} custom resource definitions.
   ...
   kubectl create -f {variables.filesUrl}/manifests/prometheus-operator-crds.yaml
   kubectl create -f {variables.filesUrl}/manifests/eck-operator-crds.yaml`}
   </CodeBlock>
...
1. Run the Helm upgrade command for `tigera-operator`:
   ...
   --namespace tigera-operator`
       ...
       : (
         `helm upgrade calico tigera-operator-{variables.chart_version_name}.tgz \\
   --set-file imagePullSecrets.tigera-pull-secret=<path/to/pull/secret>,tigera-prometheus-operator.imagePullSecrets.tigera-pull-secret=<path/to/pull/secret> \\
   --namespace tigera-operator`
       )
     }`}
...
1. Install your {variables.prodname} license.

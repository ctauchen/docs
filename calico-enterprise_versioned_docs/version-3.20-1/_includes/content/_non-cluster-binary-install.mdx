...
import NonClusterReadOnlyStep from '@site/calico-enterprise_versioned_docs/version-3.20-1/_includes/content/_non-cluster-read-only-step.mdx';
import EnvironmentFile from '@site/calico-enterprise_versioned_docs/version-3.20-1/_includes/components/EnvironmentFile';
...
1. Use the following command to download the {variables.nodecontainer} image.
   ...
   {`docker pull ${variables.registry}{variables.componentImage.cnxNode}`}
   </CodeBlock>
...
1. Create a temporary {variables.nodecontainer} container.
   ...
   <CodeBlock language='bash'>
   {`docker create --name container ${variables.registry}{variables.componentImage.cnxNode}`}
   </CodeBlock>
...
author "Project Calico Maintainers \<maintainers\.org>"

...
<CodeBlock language='bash'>
  {`{'${variables.version}' === 'master'
    ? `Invoke-WebRequest -Uri "{variables.filesUrl}/download/binaries/$\{version}/calicoctl-windows-amd64.exe" -OutFile "calicoctl.exe"`
    : `Invoke-WebRequest -Uri "{variables.downloadsurl}/ee/binaries/$\{version}/calicoctl-windows-amd64.exe" -OutFile "calicoctl.exe"`}`}
</CodeBlock>
...
1. Use the following commands to pull the `calicoctl` image from the Tigera
   ...
   <CodeBlock language='bash'>
   {`docker pull ${variables.registry}{variables.componentImage.calicoctl}`}
   </CodeBlock>

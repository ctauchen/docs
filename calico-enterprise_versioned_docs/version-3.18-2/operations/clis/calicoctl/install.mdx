...
`calicoctl` allows you to create, read, update, and delete {variables.prodname} objects
from the command line. These objects represent the networking and policy
of your cluster.

You should limit access to `calicoctl` and your {variables.prodname} datastore to
trusted administrators. We discuss methods of limiting access to the
{variables.prodname} datastore in the [configuration section](configure/index.mdx).
...
export const version = '{variables.version}' === 'master' ? 'master' : '{variables.releaseTitle}';

## How to

:::note

Make sure you always install the version of `calicoctl` that matches the version of {variables.prodname} running on your cluster.
...
<CodeBlock language='bash'>
  {'{variables.version}' === 'master'
    ? `curl -o calicoctl -O -L {variables.filesUrl}/download/binaries/${version}/calicoctl`
    : `curl -o calicoctl -O -L {variables.downloadsurl}/ee/binaries/${version}/calicoctl`}
</CodeBlock>
...
<CodeBlock language='bash'>
  {'{variables.version}' === 'master'
    ? `curl -o calicoctl -O -L {variables.filesUrl}/download/binaries/${version}/calicoctl-darwin-amd64`
    : `curl -o calicoctl -O -L {variables.downloadsurl}/ee/binaries/${version}/calicoctl-darwin-amd64`}
</CodeBlock>
...
<CodeBlock language='bash'>
  {'{variables.version}' === 'master'
    ? `Invoke-WebRequest -Uri "{variables.filesUrl}/download/binaries/${version}/calicoctl-windows-amd64.exe" -OutFile "calicocttl.exe"`
    : `Invoke-WebRequest -Uri "{variables.downloadsurl}/ee/binaries/${version}/calicoctl-windows-amd64.exe" -OutFile "calicocttl.exe"`}
</CodeBlock>
...
<CodeBlock language='bash'>
  {'{variables.version}' === 'master'
    ? `curl -o kubectl-calico -O -L {variables.filesUrl}/download/binaries/${version}/calicoctl`
    : `curl -o kubectl-calico -O -L {variables.downloadsurl}/ee/binaries/${version}/calicoctl`}
</CodeBlock>
...
<CodeBlock language='bash'>
  {'{variables.version}' === 'master'
    ? `curl -o kubectl-calico -O -L {variables.filesUrl}/download/binaries/${version}/calicoctl-darwin-amd64`
    : `curl -o kubectl-calico -O -L {variables.downloadsurl}/ee/binaries/${version}/calicoctl-darwin-amd64`}
</CodeBlock>
...
<CodeBlock language='bash'>
  {'{variables.version}' === 'master'
    ? `Invoke-WebRequest -Uri "{variables.filesUrl}/download/binaries/${version}/calicoctl-windows-amd64.exe" -OutFile "kubectl-calico.exe"`
    : `Invoke-WebRequest -Uri "{variables.downloadsurl}/ee/binaries/${version}/calicoctl-windows-amd64.exe" -OutFile "kubectl-calico.exe"`}
</CodeBlock>
...
1. Use the following commands to pull the `calicoctl` image from the Tigera
   ...
   <CodeBlock language='bash'>
   docker pull {variables.registry}%% component_image("calicoctl") %%
   </CodeBlock>
...
1. Confirm that the image has loaded by typing `docker images`.
   ...
   {variables.releases[0].components.calicoctl.image}    {variables.releases[0].components.calicoctl.version}            e07d59b0eb8a   2 minutes ago   42MB
   ```
...
<CodeBlock language='bash'>
kubectl apply -f {variables.filesUrl}/manifests/calicoctl.yaml
</CodeBlock>

:::note

You can also [view the YAML in a new tab]({variables.filesUrl}/manifests/calicoctl.yaml).

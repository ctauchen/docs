...
Learn about the new features, bug fixes, and other updates in this release of {variables.prodname}.

## New features and enhancements

### VXLAN support for cluster mesh and federation

We've expanded our support of cluster mesh to clusters using VXLAN for networking. Cluster mesh can be used to federate services and endpoints to authorize cross-cluster communication with {variables.prodname} network policies.
...
We've added support for operator-based install and upgrade for Windows nodes using HPC (HostProcess/Privileged containers). {variables.prodname} can now bootstrap Windows worker nodes just like Linux worker nodes. Platform engineers can scale Windows worker nodes with an auto-scaling node pool without bootstrapping each node manually, and upgrade {variables.prodname} deployments without manually upgrading nodes one at a time.

For more information, see [the {variables.prodnameWindows} Operator guide](../getting-started/install-on-clusters/windows-calico/operator).
...
We've made several improvements that allow you to enforce a larger number of {variables.prodname} network policies when using the eBPF dataplane.

### Security event management

We've added container threat detection events to the Security event management page in Manager UI.
Security events provides context for suspicious activity detected in your cluster. Combined with the Kubernetes context, you can see which workloads are affected.
...
* The anomaly detection feature is removed in this release. If anomaly detection is enabled and you upgrade to {variables.prodname} 3.18, you will stop receiving anomaly detection alerts. 
* [Manual install for Windows](../getting-started/install-on-clusters/windows-calico/manual-install/) will be deprecated in a future release. Starting in this release, the [standard installation is operator-based](../getting-started/install-on-clusters/windows-calico/operator).
...
* Before upgrading a {variables.prodname} cluster on MKE v3.6 to the latest {variables.prodname} version: 1) upgrade MKE from 3.6 to 3.7, then 2) upgrade {variables.prodname}.
...
To update an existing installation of {variables.prodname} 3.18, see [Install a patch release](../getting-started/manifest-archive.mdx).

...
Nutanix Kubernetes Platform (NKP) installs Calico Open Source as the CNI for managed clusters. To run $[prodname], create the managed cluster without a CNI plugin and install $[prodname] directly. See [Nutanix Kubernetes Platform](install-on-clusters/nkp.mdx).

| $[prodname] version | $[prodname] support                 | NKP versions | Kubernetes versions |
| -------------------- | ------------------------------------ | ------------ | ------------------- |
| 3.23                 | $[prodname] CNI with network policy | 2.18         | 1.34 - 1.36         |

Nutanix certifies $[prodname] on NKP. Tigera does not certify this platform independently. Direct questions about platform certification, and about which NKP versions are certified, to Nutanix.

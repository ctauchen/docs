...
This quickstart gets you a single-host Kubernetes cluster with {variables.prodname} in approximately 15 minutes.

## Value

Use this quickstart to quickly and easily try {variables.prodname} features. To deploy a cluster suitable for production, refer to [{variables.prodname} on Kubernetes](../kubernetes/index.mdx).
...
- {variables.prodname} can manage `cali` and `tunl` interfaces on the host
...
This quickstart guide uses the Tigera operator to install {variables.prodname}. The operator provides lifecycle management for Calico

...
Enable {variables.prodname} in EKS managed Kubernetes service.

## Value

EKS has built-in support for {variables.prodname}, providing a robust implementation of the full Kubernetes Network Policy API. EKS users wanting to go beyond Kubernetes network policy capabilities can make full use of the Calico Network Policy API.

You can also use {variables.prodname} for networking on EKS in place of the default AWS VPC networking without the need to use IP addresses from the underlying VPC. This allows you to take advantage of the full set of {variables.prodname} networking features, including {variables.prodname}'s flexible IP address management capabilities.

## Before you begin...

:::note

{variables.prodname} makes use of the Kubernetes Container Storage Interface (CSI) to support various types of volumes. The necessary drivers required for CSI
...
To enable {variables.prodname} network policy enforcement on an EKS cluster using the AWS VPC CNI plugin, follow these step-by-step instructions: [Installing Calico on Amazon EKS](https://docs.aws.amazon.com/eks/latest/userguide/calico.html)
...
1. Configure the {variables.prodname} installation.
...
1. Confirm installation by checking the `STATUS`, your cluster nodes should have a `Ready` status. 
   ...
   ```
### Install EKS with {variables.prodname} networking
...
{variables.prodname} networking cannot currently be installed on the EKS control plane nodes. As a result the control plane nodes
will not be able to initiate network connections to {variables.prodname} pods. (This is a general limitation of EKS's custom networking support,
not specific to {variables.prodname}.) As a workaround, trusted pods that require control plane nodes to connect to them, such as those implementing
...
1. Since this cluster will use {variables.prodname} for networking, you must delete the `aws-node` daemon set to disable AWS VPC networking for pods.

   ```bash
   kubectl delete daemonset -n kube-system aws-node
   ```

1. Now that you have a cluster configured, you can install {variables.prodname}.
...
1. Install the {variables.prodname} manifest.

   <CodeBlock language='bash'>
   kubectl apply -f {variables.manifestsUrl}/manifests/calico-vxlan.yaml
   </CodeBlock>

1. Configure {variables.prodname} to disable AWS src/dst checks.
...
1. Add {variables.prodname} into your Helm repository.

    ```batch
    helm repo add projectcalico https://docs.tigera.io/calico/charts
    ```

1. If {variables.prodname} is already added, update it to get the latest released version.

    ```batch
    helm repo update
    ```

1. Install version {variables.releaseTitle} of the {variables.prodname} operator and custom resource definitions.

    <CodeBlock language='batch'>
    helm install calico projectcalico/tigera-operator --version {variables.releaseTitle}
    </CodeBlock>
...
- [Get started with {variables.prodname} network policy](../../../network-policy/get-started/calico-policy/calico-network-policy.mdx)

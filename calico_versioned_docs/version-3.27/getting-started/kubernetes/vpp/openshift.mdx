...
Install an OpenShift 4 cluster with {variables.prodname} VPP.

## Value

Augments the applicable steps in the [OpenShift documentation](https://cloud.redhat.com/openshift/install)
to install {variables.prodname} VPP.

## How to

### Before you begin

- Ensure that your environment meets the {variables.prodname} [system requirements](../openshift/requirements.mdx) except those related to the management of `cali*`, `tunl*` and `vxlan.calico` interfaces.
...
### Update the configuration file to use {variables.prodname}
...
mkdir vpp
cd vpp
curl -O "https://raw.githubusercontent.com/projectcalico/vpp-dataplane/{variables.vppbranch}/yaml/platforms/openshift/00-namespace-calico-vpp-dataplane.yaml"
curl -O "https://raw.githubusercontent.com/projectcalico/vpp-dataplane/{variables.vppbranch}/yaml/platforms/openshift/01-cr-installation.yaml"
curl -O "https://raw.githubusercontent.com/projectcalico/vpp-dataplane/{variables.vppbranch}/yaml/platforms/openshift/03-configmap-calico-vpp-resources.yaml"
curl -O "https://raw.githubusercontent.com/projectcalico/vpp-dataplane/{variables.vppbranch}/yaml/platforms/openshift/03-role-calico-vpp-dataplane.yaml"
curl -O "https://raw.githubusercontent.com/projectcalico/vpp-dataplane/{variables.vppbranch}/yaml/platforms/openshift/03-rolebinding-calico-vpp-dataplane.yaml"
curl -O "https://raw.githubusercontent.com/projectcalico/vpp-dataplane/{variables.vppbranch}/yaml/platforms/openshift/03-serviceaccount-calico-vpp-dataplane.yaml"
curl -O "https://raw.githubusercontent.com/projectcalico/vpp-dataplane/{variables.vppbranch}/yaml/platforms/openshift/04-calico-vpp-nohuge.yaml"
cd ..
cp vpp/* manifests/
```
...
In order to customize the configuration of the VPP dataplane, edit `manifests/03-configmap-calico-vpp-resources.yaml`. For details refer to the **Configuration options** section of [Getting Started](./getting-started.mdx).
...
Once the above command is complete, you can verify {variables.prodname} VPP is installed by verifying the components are available with the following command.
...
cluster administrators manage the lifecycle of operators in their cluster. Managing the {variables.prodname}
operator with OLM gives administrators a single place to manage operators.

1. To register the running {variables.prodname} operator with OLM, first you will need to create an OperatorGroup for the operator:
...
2. Create a subscription to the operator. By subscribing to the operator package, the {variables.prodname} operator will be managed by OLM.
...
metadata:
  ...
  namespace: tigera-operator
spec:
  channel: release-{variables.tigeraOperatorVersionShort}
  installPlanApproval: Manual
  name: tigera-operator
  source: certified-operators
  sourceNamespace: openshift-marketplace
  startingCSV: tigera-operator.{variables.tigeraOperator.version}
EOF
```
...
After installing {variables.prodname} VPP, you can benefit from the features of the VPP dataplane, such as fast [IPsec](ipsec.mdx) or [Wireguard](../../../network-policy/encrypt-cluster-pod-traffic.mdx) encryption.

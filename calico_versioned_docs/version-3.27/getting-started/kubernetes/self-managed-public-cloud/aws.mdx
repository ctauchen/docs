...
Use {variables.prodname} with a self-managed Kubernetes cluster in Amazon Web Services (AWS).

## Value

Managing your own Kubernetes cluster (as opposed to using a managed-Kubernetes service like EKS), gives you the most flexibility in configuring {variables.prodname} and Kubernetes. {variables.prodname} combines flexible networking capabilities with "run-anywhere" security enforcement to provide a solution with native Linux kernel performance and true cloud-native scalability.

## Concepts

Kubernetes Operations (kops) is a cluster management tool that handles provisioning cluster VMs and installing Kubernetes. It has built-in support for using {variables.prodname} as the Kubernetes networking provider.
...
There are many ways to install and manage Kubernetes in AWS. Using Kubernetes Operations (kops) is a good default choice for most people, as it gives you access to all of {variables.prodname}’s [flexible and powerful networking features](../../../networking/index.mdx). However, there are other options that may work better for your environment.
...
To use kops to create a cluster with {variables.prodname} networking and network policy:
...
1. Configure kops to use {variables.prodname} for networking.
...
You can further customize the {variables.prodname} install with [options listed in the kops documentation](https://kops.sigs.k8s.io/networking/#calico-example-for-cni-and-network-policy).

### Other options and tools

#### Amazon VPC CNI plugin

As an alternative to {variables.prodname} for both networking and network policy, you can use Amazon’s VPC CNI plugin for networking, and {variables.prodname} for network policy. The advantage of this approach is that pods are assigned IP addresses associated with Elastic Network Interfaces on worker nodes. The IPs come from the VPC network pool and therefore do not require NAT to access resources outside the Kubernetes cluster.
...
Then install {variables.prodname} for network policy only after the cluster is up and ready.

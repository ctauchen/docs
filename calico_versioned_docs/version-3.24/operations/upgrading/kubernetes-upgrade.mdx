...
## About upgrading {variables.prodname}

This page describes how to upgrade to {variables.version} from {variables.prodname} v3.0 or later. The
procedure varies by datastore type and install method.

If you are using {variables.prodname} in etcd mode on a Kubernetes cluster, we recommend upgrading to the Kubernetes API datastore [as discussed here](../datastore-migration.mdx).

If you have installed {variables.prodname} using the `calico.yaml` manifest, we recommend upgrading to the {variables.prodname} operator, [as discussed here](../operator-migration.mdx).
...
1. Apply the {variables.version} CRDs:

   <CodeBlock language='bash'>
   kubectl apply --server-side --force-conflicts -f {variables.manifestsUrl}/manifests/operator-crds.yaml
   </CodeBlock>

1. Install the helm chart in the `tigera-operator` namespace.

   <CodeBlock language=''>
   helm install {variables.prodnamedash} projectcalico/tigera-operator --version {variables.releaseTitle} --namespace tigera-operator
   </CodeBlock>
...
1. Run the helm upgrade:
   ...
   <CodeBlock language='bash'>
   helm upgrade {variables.prodnamedash} projectcalico/tigera-operator
   </CodeBlock>

## Upgrading an installation that uses the operator

1. Download the {variables.version} operator manifest.

   <CodeBlock language='bash'>
   curl {variables.manifestsUrl}/manifests/tigera-operator.yaml -O
   </CodeBlock>
...
1. Download the {variables.version} manifest that corresponds to your original installation method.

   **{variables.prodname} for policy and networking**

   <CodeBlock language='bash'>
   curl {variables.manifestsUrl}/manifests/calico.yaml -O
   </CodeBlock>

   **{variables.prodname} for policy and flannel for networking**

   <CodeBlock language='bash'>
   curl {variables.manifestsUrl}/manifests/canal.yaml -O
   </CodeBlock>

   **{variables.prodname} for policy (advanced)**

   <CodeBlock language='bash'>
   curl {variables.manifestsUrl}/manifests/calico-policy-only.yaml -O
   </CodeBlock>
...
1. Use the following command to initiate a rolling update, after replacing
   `<manifest-file-name>` with the file name of your {variables.version} manifest.
...
1. Watch the status of the upgrade as follows.
   ...
   Verify that the status of all {variables.prodname} pods indicate `Running`.

   ```
   {variables.noderunning}-hvvg8     2/2   Running   0    3m
   {variables.noderunning}-vm8kh     2/2   Running   0    3m
   {variables.noderunning}-w92wk     2/2   Running   0    3m
   ```
...
1. Use the following command to check the {variables.prodname} version number.

   ```bash
   calicoctl version
   ```

   It should return a `Cluster Version` of `{variables.version}.x`.

1. If you have [enable application layer policy](../../network-policy/istio/app-layer-policy.mdx),
   follow [the instructions below](#upgrading-if-you-have-application-layer-policy-enabled) to complete your upgrade. Skip this if you are not using Istio with {variables.prodname}.
...
1. Congratulations! You have upgraded to {variables.prodname} {variables.version}.
...
1. Download the {variables.version} manifest that corresponds to your original installation method.
   ...
   **{variables.prodname} for policy and networking**
   ...
   <CodeBlock language='bash'>
   curl {variables.manifestsUrl}/manifests/calico-etcd.yaml -O
   </CodeBlock>

   **{variables.prodname} for policy and flannel for networking**

   <CodeBlock language='bash'>
   curl {variables.manifestsUrl}/manifests/canal-etcd.yaml -O
   </CodeBlock>

   :::note

   You must manually apply the changes you made to the manifest
   during installation to the downloaded {variables.version} manifest. At a minimum,
...
1. Watch the status of the upgrade as follows.
   ...
   {variables.noderunning}-hvvg8                          1/2       Running   0          3m
   {variables.noderunning}-vm8kh                          1/2       Running   0          3m
   {variables.noderunning}-w92wk                          1/2       Running   0          3m
   ```

   :::tip

   The {variables.noderunning} pods will report `1/2` in the `READY` column, as shown.
...
1. Use the following command to check the {variables.prodname} version number.
   ...
   It should return a `Cluster Version` of `{variables.version}`.
...
Dikastes is versioned the same as the rest of {variables.prodname}, but an upgraded `calico-node` will still be able to work with a downlevel Dikastes
...
take the following steps to upgrade the Dikastes sidecars running in your application pods. Skip these steps if you are not using Istio with {variables.prodname}.
...
1. Update the Istio sidecar injector template to use the new version of Dikastes. Replace `{'<your Istio version>'}` below with
   ...
   <CodeBlock language='bash'>
   kubectl apply -f {variables.manifestsUrl}/manifests/alp/istio-inject-configmap-{'<your Istio version>'}.yaml
   </CodeBlock>

...
Perform common customizations of a {variables.prodname} installation.
...
For many environments, {variables.prodname} will auto-detect the correct pod IP range to use, or select an unused range on the cluster.
...
the ipPools array can take at most one IPv4 and one IPv6 CIDR, and only takes effect when installing {variables.prodname} for the first
...
We provide a number of manifests to make deployment of {variables.prodname} easy. You can optionally
modify the manifests before applying them. Or you can modify the manifest and reapply it to change
settings as needed.

### About customizing {variables.prodname} manifests

Each manifest contains all the necessary resources for installing {variables.prodname}
on each node in your Kubernetes cluster.

It installs the following Kubernetes resources:

- Installs the `{variables.nodecontainer}` container on each host using a DaemonSet.
- Installs the {variables.prodname} CNI binaries and network config on each host using a DaemonSet.
...
{variables.prodname} IPAM assigns IP addresses from [IP pools](../../../reference/resources/ippool.mdx).
...
[Configuring {variables.nodecontainer}](../../../reference/configure-calico-node.mdx).
...
manifest. For more information, see [Configuring {variables.nodecontainer}](../../../reference/configure-calico-node.mdx).
...
For more information on {variables.nodecontainer}'s configuration variables, including additional VXLAN settings, see
[Configuring {variables.nodecontainer}](../../../reference/configure-calico-node.mdx).

:::note

The `CALICO_IPV4POOL_VXLAN` environment variable only takes effect when the first {variables.nodecontainer} to start
...
| etcd_ca        | The file containing the root certificate of the CA that issued the etcd server certificate. Configures `{variables.nodecontainer}`, the CNI plugin, and the Kubernetes controllers to trust the signature on the certificates provided by the etcd server.  | None                  |
| etcd_key       | The file containing the private key of the `{variables.nodecontainer}`, the CNI plugin, and the Kubernetes controllers client certificate. Enables these components to participate in mutual TLS authentication and identify themselves to the etcd server. | None                  |
| etcd_cert      | The file containing the client certificate issued to `{variables.nodecontainer}`, the CNI plugin, and the Kubernetes controllers. Enables these components to participate in mutual TLS authentication and identify themselves to the etcd server.          | None                  |

To use these manifests with a TLS-enabled etcd cluster you must do the following:

1. Download the {variables.version} manifest that corresponds to your installation method.
...
{variables.prodname}'s manifests assign its components one of two service accounts.
...
an additional lightweight sidecar called Dikastes which receives {variables.prodname} policy
...
```yaml
      - name: dikastes
        image: {variables.registry}{variables.imageNames.calico/dikastes}:{variables.releases[0].components.calico/dikastes.version}
...
Refer to the
[Calico ConfigMap manifest]({variables.manifestsUrl}/manifests/alp/istio-inject-configmap-1.4.2.yaml) for an

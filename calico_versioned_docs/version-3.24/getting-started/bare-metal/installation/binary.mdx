...
1. Use the following command to download the {variables.nodecontainer} image.
   ...
   docker pull {variables.nodecontainer}:{variables.releases[0].components['calico/node'].version}
   </CodeBlock>
...
1. Confirm that the image has loaded by typing `docker images`.
   ...
   {variables.nodecontainer}      {variables.releases[0].components['calico/node'].version}        e07d59b0eb8a   2 minutes ago   42MB
   ```

1. Create a temporary {variables.nodecontainer} container.

   <CodeBlock language='bash'>
   docker create --name container {variables.nodecontainer}:{variables.releases[0].components['calico/node'].version}
   </CodeBlock>

...
export function buildUrl() {
  const url =
    variables.releaseTitle === 'master'
      ? 'https://github.com/projectcalico/calico/releases/latest/download'
      : `https://github.com/projectcalico/calico/releases/download/${variables.releaseTitle}`;
  return url;
}
...
The `calicoctl` command line tool is required to use many of {variables.prodname}'s features. It
...
To manage {variables.prodname} APIs in the `projectcalico.org/v3` API group, you should use `calicoctl`. This is because

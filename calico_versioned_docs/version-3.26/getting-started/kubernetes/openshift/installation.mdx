...
Once the above command is complete, you can verify {variables.prodname} is installed by verifying the components are available with the following command.
...
In order to register the running {variables.prodname} operator with OLM, first you will need to create an OperatorGroup for the operator:
...
Next, you will create a Subscription to the operator. By subscribing to the operator package, the {variables.prodname} operator will be managed by OLM.

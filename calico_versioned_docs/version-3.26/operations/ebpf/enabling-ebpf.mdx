...
import EbpfValue from '@site/calico_versioned_docs/version-3.26/_includes/content/_ebpf-value.mdx';
...
Enable the eBPF data planeon an existing cluster.
...
- Distributions:
  ...
  - AKS with limitations:
    ...
    - [AKS with {{prodname}} networking](../../getting-started/kubernetes/managed-public-cloud/aks.mdx#install-aks-with-{{prodnamedash}}-networking) is in testing with the eBPF data plane . This should be a better solution overall but, at time of writing, the testing was not complete.
...
- Clusters with some eBPF nodes and some standard data planeand/or Windows nodes.
- IPv6.
- Floating IPs.

...
This guide helps you install the `calicoctl` command line tool to manage {variables.prodname} resources
and perform administrative functions.

## Value

The `calicoctl` command line tool is required in order to use many of {variables.prodname}'s features. It
is used to manage {variables.prodname} policies and configuration, as well as view detailed cluster status.

## Concepts

### API groups

All Kubernetes resources belong to an API group. The API group is indicated by the resource's `apiVersion`. For example, {variables.prodname}
...
In order to manage {variables.prodname} APIs in the `projectcalico.org/v3` API group, you should use `calicoctl`. This is because
...
In addition to resource management, `calicoctl` also enables other {variables.prodname} administrative tasks such as viewing IP pool utilization
and BGP status.
...
{variables.prodname} objects are stored in one of two datastores, either etcd or Kubernetes. The choice of datastore is determined at the time {variables.prodname}
is installed. Typically for Kubernetes installations the Kubernetes datastore is the default.

You can run `calicoctl` on any host with network access to the {variables.prodname} datastore as either a binary or a container.
...
docker pull {variables.registry}{variables.imageNames.calicoctl}:{variables.releaseTitle}
</CodeBlock>
...
- **etcd**
  ...
  <CodeBlock language='bash'>
  kubectl apply -f {variables.manifestsUrl}/manifests/calicoctl-etcd.yaml
  </CodeBlock>

  :::note

  You can also
  [view the YAML in a new tab]({variables.manifestsUrl}/manifests/calicoctl-etcd.yaml).
...
- **Kubernetes API datastore**
  ...
  <CodeBlock language='bash'>
  kubectl apply -f {variables.manifestsUrl}/manifests/calicoctl.yaml
  </CodeBlock>

  :::note

  You can also
  [view the YAML in a new tab]({variables.manifestsUrl}/manifests/calicoctl.yaml).

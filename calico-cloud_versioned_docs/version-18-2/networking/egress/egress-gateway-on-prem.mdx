...
{variables.prodname}'s own policy (including [DNS policy](../../network-policy/domain-based-policy.mdx)) and
...
{variables.prodname} allows specifying an [IP pool](../../reference/resources/ippool.mdx) for each pod or namespace, and
...
Kubernetes **Namespace** and **Pod** resources with these {variables.prodname} annotations:
...
1.  Impose the cluster's own bespoke scheme for a Namespace or Pod to identify the egress gateways
    that it wants to use, less general than {variables.prodname}'s egress annotations. Then the
    ...
    through after adding the corresponding {variables.prodname} egress annotations.
...
{variables.prodname} and other ingress traffic is blocked. That means that there are effectively two places where
...
Since v3.15.0, {variables.prodname} also sends health probes to the egress gateway pods from the nodes where
...
- Remote pods to check if the egress gateway is "ready". Only "ready" egress
  gateways will be used for remote client traffic. This traffic is automatically allowed by {variables.prodname} and
  no policy is required to allow it. {variables.prodname} only sends probes to egress gateway pods that have a named
...
In a {variables.prodname} deployment, the Kubernetes namespace and pod resources honor annotations that
...
{variables.prodname} [network policy](../../reference/resources/networkpolicy.mdx#entityrule).
...
1. `gatewayPreference` specifies hints to the gateway selection process. The default `None`, selects the default selection process. If set to `PreferNodeLocal`, then egress gateways local to the client's node are used if available. If there are no local egress gateways, {variables.prodname} uses other egress gateways. In this example, for the default route, egress gateways local to the client's node are used if present. If not, all egress gateways matching the selector are used.
...
{variables.prodname} rejects egress gateway policies that do any of the following:
...
If you attempt to apply an egress gateway policy that has not been created, {variables.prodname} will block all traffic from the namespace or pod.
...
- Use `kubectl apply` to create the egress gateway resource. Tigera Operator will read the newly created resource and wait
  for the other {variables.prodname} components to be upgraded. Once the other {variables.prodname} components are upgraded, Tigera Operator

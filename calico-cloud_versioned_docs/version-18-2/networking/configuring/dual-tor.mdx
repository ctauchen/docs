...
**{variables.nodecontainer}** run as a container, pre-Kubernetes
...
{variables.prodname}'s {variables.nodecontainer} image can be run in an "early networking" mode,
...
{variables.nodecontainer} _pod_:

- Initially the {variables.nodecontainer} pod does not yet exist, so the early networking
...
- After Kubernetes has started on the node, and Calico has been installed in Kubernetes,
  the {variables.nodecontainer} pod runs and starts its own BGP service. The early
...
- Later, the {variables.nodecontainer} pod might be shut down, e.g. for restart or upgrade.
...
designs](../../reference/architecture/design/l3-interconnect-fabric.mdx). One of
...
- [Arrange for dual-homed nodes to run {variables.nodecontainer} on each boot](#arrange-for-dual-homed-nodes-to-run-cnx-node-on-each-boot)
- [Configure your ToR routers and infrastructure](#configure-your-tor-routers-and-infrastructure)
- [Install Kubernetes and {variables.prodname}](#install-kubernetes-and-calico-enterprise)
...
1.  Prepare BGPPeer resources to specify how each node in your cluster should peer with
    ...
    - `failureDetectionMode: BFDIfDirectlyConnected` to enable BFD, when possible, for
      ...
      {variables.prodname} only supports BFD on directly connected peerings, but
    ...
    - `birdGatewayMode: DirectIfDirectlyConnected` to enable the "direct" next hop
      ...
      "recursive"](https://bird.network.cz/?get_doc&v=16&f=bird-6.html#ss6.3).
      ...
      keep" for all routes, with "gateway direct" on the {variables.prodname} nodes. In
...
1.  Prepare this BGPConfiguration resource to [disable the full node-to-node
    mesh](bgp.mdx#disable-the-default-bgp-node-to-node-mesh):
...
1.  Prepare disabled IPPool resources for the CIDRs from which you will allocate stable
    ...
    These IPPool definitions tell {variables.prodname}'s BGP component to export
    routes within the given CIDRs, which is essential for the core BGP infrastructure to
    learn how to route to each stable address. `disabled: true` tells {variables.prodname}
...
1.  Prepare an EarlyNetworkConfiguration resource to specify the additional information
    ...
    - Its BGP AS number.
    - The IPs that the node should peer with, when {variables.nodecontainer} runs
    ...
    - Any labels that the node should have, so as to match the right BGPPeer definitions
      for its rack, when {variables.nodecontainer} runs as a Kubernetes pod.
...
EarlyNetworkingConfiguration supplies labels and AS numbers to apply to each
...
### Arrange for dual-homed nodes to run {variables.nodecontainer} on each boot

{variables.prodname}'s {variables.nodecontainer} image normally runs as a Kubernetes pod, but
...
podman run --privileged --net=host \
    ...
    {variables.registry}{variables.imageNames.node}:latest
```
...
{variables.nodecontainer} container.
...
ExecStart=/bin/podman run --rm --privileged --net=host --name=calico-early -v /etc/calico-early:/etc/calico-early -e CALICO_EARLY_NETWORKING=/etc/calico-early/details.yaml {variables.registry}{variables.imageNames.node}:latest
[Install]
...
- You must also install your Tigera-issued pull secret at `/root/.docker/config.json`,
  on each node, to enable pulling from {variables.registry}.
...
{variables.prodname} nodes, to reflect routes between the nodes in that rack, and to
propagate routes between the ToR routers in different racks. In addition we recommend
consideration of the following points.

BFD should be enabled if possible on all BGP sessions - both to the {variables.prodname}
...
{variables.prodname} nodes, and between racks in your core infrastructure. Traditional BGP
...
### Install Kubernetes and {variables.prodname}
...
{variables.nodecontainer} pods start running on any node. We can illustrate this by looking
...
Now continue with the {variables.prodname} install process, and you should observe each node

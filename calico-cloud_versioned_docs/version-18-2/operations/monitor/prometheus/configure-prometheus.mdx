...
As an example, to fire a alert when a {variables.noderunning} instance has been down for
...
The Alerting Rules installed by the {variables.prodname} install manifest is a simple
...
spec:
  ...
  version: {variables.releases[0].components.prometheus.version}
  retention: 1h
  resources:
    requests:
      memory: 800Mi
...
The scrape interval of endpoints ({variables.noderunning} in our case) is defined as part of
...
To update {variables.prodname} Prometheus' scrape interval to 10 seconds modify the manifest
to this:

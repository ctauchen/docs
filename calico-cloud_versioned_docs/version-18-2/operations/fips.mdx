...
Run {variables.prodname} in [FIPS 140-2](https://csrc.nist.gov/publications/detail/fips/140/2/final) compliant mode.

## Value

When running in FIPS compliant mode, {variables.prodname} uses FIPS-approved cryptographic algorithms and NIST-validated cryptographic modules.
...
agencies and government contractors. {variables.prodname} FIPS mode is enabled during installation by:
...
- {variables.prodname} contains programs that run directly on the host that use dynamic linking of c libraries. For this reason,
...
To install {variables.prodname} in FIPS mode follow these steps.

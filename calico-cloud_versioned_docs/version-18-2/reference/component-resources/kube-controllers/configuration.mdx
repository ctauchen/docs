...
## The {variables.imageNames.kubeControllers} container

The `{variables.imageNames.kubeControllers}` container includes the following controllers:
...
The federation controller syncs Kubernetes federated endpoint changes to the {variables.prodname} datastore.
...
This controller is valid for all {variables.prodname} datastore types. For more details refer to the

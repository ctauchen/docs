...
### New flow logs panel for the Endpoints page

We've add a link to view flow logs for a selected endpoint on the Endpoints page in Manager UI.
...
<!-- BELOW ARE 3.18 EP2 features -->

### Support for Kubernetes v1.28

To see the Kubernetes version supported for your platform by release, see [Support and compatibility](../getting-started/compatibility).

### VXLAN encapsulation for cross-cluster connectivity

Added support for VXLAN overlay networking to federated endpoint identity and services. Using VXLAN overlay networking alleviates the work of configuring routable pod CIDRs for cross-cluster connectivity, and provides a solution if IPs are exhausted. {variables.prodname} uses the VXLAN overlay network to establish pod CIDR routes to remote nodes.
...
We've added supported for Azure CNI with overlay networking for AKS clusters, which is useful if you've exhausted your IP addresses.
This option augments support for the Azure CNI with no overlay (where a VNET IP address is assigned to every pod).

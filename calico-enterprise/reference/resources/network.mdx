...
A gateway address is the `nextHop` of a route declared on that subnet, so each distinct `nextHop` is reserved, and a subnet with no routes has no reserved gateway address.

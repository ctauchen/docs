...
- Either {{prodname}}'s eBPF mode or `kube-proxy` must be active on a host for services to function. If you
  ...
  detects that the kernel is not supported, it will fall back to standard data planemode (which does not support
  services).
  ...
  ```bash
  BPF data planemode enabled but not supported by the kernel.  Disabling BPF mode.
  ```
...
- Run the tool as follows:
  ...
  Also, it is possible to fetch various counters, like packets dropped by a policy or different errors, from BPF data planeusing the same tool.
...
- If your cluster is very large, or your workload involves significant service churn, you can increase the interval
  at which Felix updates the services data planeby increasing the `bpfKubeProxyMinSyncPeriod` setting. The default is

...
By default, {variables.prodname} uses an IPAM block size of 64 addresses – /26 for IPv4, and /122 for IPv6. However, the block size can be changed depending on the IP pool address family.
...
By default, the {variables.prodname} IPAM block size for an IP pool is /26. To expand from the default size /26, lower the `blockSize` (for example, /24). To shrink the `blockSize` from the default /26, raise the number (for example, /28).

### Best practice: change IP pool block size before installation

Because the `blockSize` field cannot be edited directly after {variables.prodname} installation, it is best to change the IP pool block size before installation to minimize disruptions to pod connectivity.

## Before you begin...

**Required**

Verify that you are using {variables.prodname} IPAM.

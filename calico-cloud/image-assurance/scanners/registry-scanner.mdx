...
The registry scanner comes in a Docker image. To get the image, run this command: `docker pull quay.io/tigera/image-assurance-registry-scanner:{variables.imageassuranceversion}`.
...
{`docker run -e ... -v ~/.docker/config.json:/.docker/config.json quay.io/tigera/image-assurance-registry-scanner:${variables.imageassuranceversion}`}
</CodeBlock>
...
### Scan images and send results to {variables.prodname}

Example: gcr registry with Docker credentials

<CodeBlock language='bash'>
{`docker run -e REGISTRY=gcr.io -e IMAGE_ASSURANCE_API_URL=https://\<>-management.dev.calicocloud.io/bast -e IMAGE_ASSURANCE_API_TOKEN=$TOKEN -e DOCKER_USERNAME=\<> -e DOCKER_PASSWORD=\<> quay.io/tigera/image-assurance-registry-scanner:${variables.imageassuranceversion}`}
</CodeBlock>

Example: acr registry with Azure credentials

<CodeBlock language='bash'>
{`docker run -e REGISTRY=your-org.azurecr.io -e IMAGE_ASSURANCE_API_URL=https://\<>-management.dev.calicocloud.io/bast -e IMAGE_ASSURANCE_API_TOKEN=$TOKEN -e AZURE_CLIENT_ID=\<> -e AZURE_CLIENT_SECRET=\<> -e AZURE_TENANT_ID=\<> quay.io/tigera/image-assurance-registry-scanner:${variables.imageassuranceversion}`}
</CodeBlock>

Example: ecr registry with AWS credentials

<CodeBlock language='bash'>
{`docker run -e REGISTRY=gcr.io -e IMAGE_ASSURANCE_API_URL=https://\<>-management.dev.calicocloud.io/bast -e IMAGE_ASSURANCE_API_TOKEN=$TOKEN -e AWS_ACCESS_KEY_ID=\<> -e AWS_SECRET_ACCESS_KEY=\<> -e AWS_REGION=\<>  quay.io/tigera/image-assurance-registry-scanner:${variables.imageassuranceversion}`}
</CodeBlock>

Example: run when mounting the dockerfile

<CodeBlock language='bash'>
{`docker run -e REGISTRY=gcr.io -e IMAGE_ASSURANCE_API_URL=https://\<>-management.dev.calicocloud.io/bast -e IMAGE_ASSURANCE_API_TOKEN=$TOKEN -v ~/.docker/config.json/:/.docker/config.json quay.io/tigera/image-assurance-registry-scanner:${variables.imageassuranceversion}`}
</CodeBlock>

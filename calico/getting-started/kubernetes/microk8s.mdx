...
Install a single node MicroK8s cluster with {variables.prodname} in approximately 5 minutes.

## Value

MicroK8s is a lightweight upstream Kubernetes distribution package to run as an immutable container.

Use this quickstart to quickly and easily try {variables.prodname} features with MicroK8s.
